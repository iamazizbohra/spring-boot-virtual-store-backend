...
public class AutheticationController {
	...
	@PostMapping("/login")
	public ResponseEntity<SuccessResponseDto> login(HttpServletRequest request, @RequestBody LoginRequestDto payload) {
		Set<ConstraintViolation<LoginRequestDto>> violations = validator.validate(payload);
		...
		SuccessResponseDto successResponseDto = SuccessResponseDto.builder().timestamp(LocalDateTime.now()).status(200)
				.message("Login Successful").data(jwtTokenDto).path("uri=" + request.getServletPath()).build();
